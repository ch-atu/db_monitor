...
IGNORE_FS = {'binfmt_misc', 'cgroup', 'debugfs', 'devpts', 'devtmpfs', 'fusectl', 'proc', 'pstore', 'securityfs',
...
class LinuxStat(LinuxBase):
    ...
    def get_linux_stat(self):
        ...
        linux_stat['cpuinfo'] = self.get_cpu_info()  # cpu基本信息
        linux_stat['Memtotal'] = self.get_memtotal()  # 获取总内存
        linux_stat['ipinfo'] = self.get_host_ip()  # 获取ip信息
        linux_stat['load'] = self.get_load()  # 1、5、15分钟内的平均进程数
        linux_stat['cpu'] = self.get_cpu_stat()  # 获取cpu状态
    ...
    def get_mem_stat(self):
        ...
        # - self.val['SReclaimable'] + self.val['Shmem']
    ...
    def get_stat(self, stat_name, replace=None):
        ...
        res = super().exec_command(command,self.conn)
        fd = res
        fd_readlines = fd.readlines()
        for l in fd_readlines:
