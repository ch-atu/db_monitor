...
def check_linux(tags, linux_params):
    ...
    try:
        ...
        insert_data_sql = "insert into linux_stat(" \
                          "tags,host,port,hostname,ipinfo,linux_version,updays,kernel," \
                          "frame,cpu_mode,cpu_cache,processor,cpu_speed," \
                          "recv_kbps,send_kbps,load1,load5,load15,cpu_sys,cpu_" \
                          "iowait,cpu_user,cpu_used,memtotal,mem_used,mem_cache,mem_buffer,mem_free,mem_used_mb," \
                          "swap_used,swap_free,swapin,swapout,pgin,pgout,pgfault," \
                          "pgmjfault,tcp_close,tcp_timewait,tcp_connected,tcp_syn,tcp_listen,iops,read_mb,write_mb," \
                          "proc_new,proc_running,proc_block,intr,ctx,softirq,status,check_time" \
                          ") " \
                          "values (" \
                          "'{tags}','{host}',{port},'{hostname}','{ipinfo}','{linux_version}'," \
                          "{updays},'{kernel}','{frame}','{cpu_mode}','{cpu_cache}','{processor}','{cpu_speed}'," \
                          "{recv_kbps},{send_kbps},{load1},{load5},{load15},{cpu_sys},{cpu_iowait}," \
                          "{cpu_user},{cpu_used},{memtotal},{mem_used}," \
                          "{mem_cache},{mem_buffer},{mem_free},{mem_used_mb}," \
                          "{swap_used},{swap_free},{swapin},{swapout},{pgin},{pgout}," \
                          "{pgfault},{pgmjfault},{tcp_close},{tcp_timewait},{tcp_connected}," \
                          "{tcp_syn},{tcp_listen},{iops},{read_mb},{write_mb}," \
        ...
        for each in diskfree_list:
            ...
            insert_data_sql = '''insert into linux_disk(
            tags,host,dev,total_size,used_size,free_size,used_percent,mount_point,check_time
            ) 
        ...
        for each in iostat:
            insert_data_sql = "insert into linux_io_stat(" \
                              "tags,host,dev,rd_s,rd_avgkb,rd_m_s,rd_mrg_s,rd_cnc,rd_rt,wr_s," \
                              "wr_avgkb,wr_m_s,wr_mrg_s,wr_cnc,wr_rt,busy,in_prg,io_s,qtime,stime,check_time" \
                              ")" \
                              " values (" \
                              "'{tags}','{host}','{dev}',{rd_s},{rd_avgkb},{rd_m_s},{rd_mrg_s},{rd_cnc},{rd_rt},{wr_s}," \
                              "{wr_avgkb},{wr_m_s},{wr_mrg_s},{wr_cnc},{wr_rt},{busy}," \
                              "{in_prg},{io_s},{qtime},{stime},'{check_time}'" \
                              ")"
        ...
        clear_table(tags,'linux_stat')
        sql = "insert into " \
              "linux_stat(tags,host,port,status,check_time) " \
