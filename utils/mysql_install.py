...
class MysqlInstall():
    ...
    def generate_mysql_cnf(self, linux_conn, linux_params):
        ...
        cnf_template = '{}my.cnf.template.5.7'.format(self.local_path) if self.node_info[
    ...
    def mysql_initialize(self, linux_conn, linux_params):
        ...
        LinuxBase(linux_params).sftp_upload_file('{}{}'.format(self.local_path, mysql_package),
        ...
        self.log('开始初始化MySQL..')
        # 初始化
        cmd = '{}/bin/mysqld --defaults-file={}/my.cnf --initialize-insecure --user=mysql'.format(mysql_path,
                                                                                                  mysql_path)
        LinuxBase(linux_params).exec_command_res(cmd, linux_conn)
        self.log('{}：MySQL初始化完成'.format(self.node_info['ip']))
        self.log('请使用MySQL用户(默认密码mysqld)启动MySQL数据库：{}/bin/mysqld_safe --defaults-file={}/my.cnf --user=mysql &'.format(
